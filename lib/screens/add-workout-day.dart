...
class _AddWorkoutDayState extends State<AddWorkoutDay> {
  ...
  void _saveDayPlan() {
    if (_fbKey.currentState.saveAndValidate()) {
      if (day.drillBlocks.length > 0 &&
          day.drillBlocks.where((d) => d is WorkoutRestDrillBlock).length ==
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AddWorkoutAppBar(
        titleText: 'Create Day Plan',
        onPressCallback: _saveDayPlan,
      ),
              ...
              ListTile(
                ...
                title: Row(
                  children: <Widget>[
                    ...
                    canSelectDrillsCount(block)
                        ? DropdownButton(
                            items: Iterable<int>.generate(5)
                                .map((val) => DropdownMenuItem(
                                      value: isMultisetWorkout(block)
                                          ? val + 2
                                          : val + 1,
                                      child: Text(
                                          '${isMultisetWorkout(block) ? (val + 2) : (val + 1)} Drills'),
                                    ))
                                .toList(),
                            onChanged: (val) {
                              setState(() {
  ...
  Color _getDrillBlockHeaderColor(
    ...
    if (block is WorkoutRestDrillBlock) {
      return Color.fromRGBO(80, 150, 70, 0.9);
    }
    return Color.fromRGBO(50, 65, 85, 0.9);
  }

  String _getDrillBlockHeader(WorkoutAmrapDrillBlock block) {
    if (block is WorkoutAmrapDrillBlock) {
      return 'AMRAP';
    }
    ...
    if (block is WorkoutMultisetDrillBlock) {
      return 'Multi Drill';
    }
    if (block is WorkoutEmomDrillBlock) {
      return 'EMOM';
    }
    if (block is WorkoutSingleDrillBlock) {
      return 'Single Drill';
    }
    if (block is WorkoutRestDrillBlock) {
      return 'Rest';
